...
def extract_feature_gallery(model_path='./models/net_best.pth',data_path='../dataset/data',bach_size=256,use_gpu=True):
    ...
    for img, path in data_loader:
        ...
        path_list += list(path)
    pickle.dump(features, open('../results/features/gallery_'+data_folder_name+'_feature.pkl', 'wb'))
    pickle.dump(path_list, open('../results/features/gallery_image_'+data_folder_name+'_paths.pkl', 'wb'))

def get_saved_feature(path='../../yolov5-6.0/features/'):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gallery_data_path',default='../dataset/seg_origin_data', help="gallery data path")
    parser.add_argument('--use_gpu', default=True, help="if use gpu ")
    args = parser.parse_args()
    # gallery feature
    extract_feature_gallery(args.model_path,args.gallery_data_path,args.batch_size,args.use_gpu)
