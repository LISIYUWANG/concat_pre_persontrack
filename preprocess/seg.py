...
class DeepLabModel(object):
  """Class to load deeplab models and run inference."""
  ...
  def __init__(self, tarball_path):
    """Creates and loads pretrained deeplab models."""
    self.graph = tf.Graph()
...
def run_visualization(filepath):
  """Inferences DeepLab models and visualizes result."""
  try:
...
if __name__ == '__main__':
    ...
    path = 'D:/project/airport/TrackDetect-master/yolov5-6.0/runs/detect/exp33/crops/person'
    #"../dataset/origin_data"
    parser = argparse.ArgumentParser(description="Segment image")
    # data
    parser.add_argument('--modelType',  default='xception_model',help='models name')
    parser.add_argument('--inputRootPath',default=path, help="input data path")
    parser.add_argument('--outputRootPath', default="../dataset/seg_origin_data", help="input data path")
    args = parser.parse_args()


    rootPath = sys.argv[0]
    inputRootPath = args.inputRootPath # "../../LWDZ/Market_1501/query"
    outputRootPath =args.outputRootPath # "../../LWDZ/Market_1501/query_removal"
    ...
    print('models loaded successfully : ' + modelType)
...
'''
  sys_temp = [sys.argv[0], 'aoe.jpg', 'aoe.png', "1"]

  inputFilePath = sys_temp[1]
  outputFilePath = sys_temp[2]

  modelType = "mobile_net_model"
  if len(sys_temp) > 3 and sys_temp[3] == "1":
